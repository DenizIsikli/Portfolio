      ...
      <div className="container">
        ...
        <div className="flex justify-center mt-10">
          <Dialog.Root>
            <Dialog.Trigger asChild>
              <motion.button
                className="mt-4 inline-flex h-10 items-center rounded-md border border-border/50 px-4 text-sm bg-gradient-to-r from-primary/10 to-purple-500/10 hover:from-primary/20 hover:to-purple-500/20 text-foreground transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring hover:scale-105"
                aria-label="View project details"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                <FaExternalLinkAlt className="mr-2" /> View More Projects
              </motion.button>
            </Dialog.Trigger>
            <Dialog.Portal>
              <Dialog.Overlay className="fixed inset-0 bg-black/60 backdrop-blur-sm" />
              ...
              >
                ...
                <div className="mt-6 max-h-[65vh] overflow-y-auto">
                  <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3 justify-items-center">
                    {projects.slice(6).map((proj, idx) => (
                      <div
                        key={idx}
                        className="w-full max-w-sm rounded-lg border border-border/50 p-4 text-center"
                      >
                        ...
                        <div className="mt-4 flex gap-2">
                          {proj.links.code && (
                            <motion.a
                              href={proj.links.code}
                              className="inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-md border border-border/50 px-3 text-sm bg-gradient-to-r from-primary/10 to-purple-500/10 hover:from-primary/20 hover:to-purple-500/20 transition-all duration-300"
                              target="_blank"
                              rel="noopener noreferrer"
                              whileHover={{ scale: 1.05 }}
                              whileTap={{ scale: 0.95 }}
                          ...
                          <Dialog.Root>
                            <Dialog.Trigger asChild>
                              <motion.button
                                className="inline-flex h-10 flex-1 items-center justify-center rounded-md border border-border/50 px-3 text-sm bg-gradient-to-r from-primary/10 to-purple-500/10 hover:from-primary/20 hover:to-purple-500/20 transition-all duration-300"
                                aria-label={`View details for ${proj.title}`}
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                              >
                            ...
                            <Dialog.Portal>
                              ...
                              <Dialog.Content className="fixed left-1/2 top-1/2 w-[90vw] max-w-2xl -translate-x-1/2 -translate-y-1/2 rounded-xl border border-border/50 bg-card/95 backdrop-blur-md p-6 shadow-2xl focus:outline-none">
                                ...
                                <Dialog.Close asChild>
                                  <motion.button
                                    className="absolute right-4 top-4 inline-flex h-8 w-8 items-center justify-center rounded-md border border-border/50 hover:bg-muted/50 transition-all duration-300"
                                    whileHover={{ scale: 1.1, rotate: 90 }}
                                    whileTap={{ scale: 0.9 }}
                                  >
                                    ×
